...
export class CreateAccountComponent implements OnInit {
  ...
  checkSignupToken() {
    ...
    let tokenData: SignupTokenData = this.tokenService.getTokenData<SignupTokenData>(this.signupToken);
    this.apiService.checkAccountExistence(tokenData.email).subscribe((respone: any) => {
      if (respone.data == true) {
        this.notificationService.Info(AppConstants.notificationMessages.accountCreated);
  ...
  onSubmit() {
    this.authService.createAccount(this.accountForm.value, this.signupToken);
  }
}
