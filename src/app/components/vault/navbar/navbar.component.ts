...
export class NavbarComponent implements OnInit {
  ...
  ngOnInit(): void { 
    ...
    if (this.isUserLoggedIn) this.tokenData = this.cryptographyService.getDecryptedAccessToken();
  }

  async onLogoutButtonClick() {
   this.dialogService.getConfirmation(AppConstants.dialogMessages.logoutConfirmation)
    .subscribe((confirmation: boolean) => {
    if (confirmation) this.authService.logout();
    });
  }
}
